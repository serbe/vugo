<template>
  <div>
    <p>
      <a class="button mb1" href="/post/">Добавить</a>
    </p>
    <p class="control">
    ...
    <table class="table is-striped fixed_table">
      ...
      <tbody>
        <tr v-for="(item, idx) in posts" @click="onClick(item)" class="link">
          <td class="tvm">{{ item.name }}</td>
          <td class="has-text-centered tvm"><input type="checkbox" :checked="item.go" disabled></td>
  ...
  export default {
    ...
    methods: {
      ...
      filterPosts () {
        ...
        let posts = this.postsList.filter((f) => {
          return queryArr.every(e => f.str.includes(e))
        })
        this.posts = posts.filter((f, i) => {
