<template>
  <div>
    <p>
      <a class="button mb1" href="/department/">Добавить</a>
    </p>
    <p class="control">
    ...
    <table class="table is-striped fixed_table">
      ...
      <tbody>
        <tr v-for="(item, idx) in departments" @click="onClick(item)" class="link">
...
<script>
  export default {
    ...
    methods: {
      onClick (item) {
        this.$router.push('/department/' + item.id)
      },
      ...
      filterDepartments () {
        ...
        let departments = this.departmentsList.filter((f) => {
          return queryArr.every(e => f.str.includes(e))
        })
        this.departments = departments.filter((f, i) => {
